---
title: "User Manual and Tutorial for GWAS"
...
In order to truly show how the using the covariates work to improve the GWAS accuracies I am going to simulate some QTN effects using the function G2P. All this function is doing is generating effects and randomly assigning then to different SNPs. This will allow me to run GWAStest and be able to determine it’s accuracy 

As we can see in the Manhattan plot and from the printed massages 8 significant SNPs were detected with 4 of then in black are true positives 2 in blue are false positives and the two in red are false negatives. Note some of the lines are very close to each other and can be hard to see which is why it’s nice to have messages = TRUE as then prints the important findings in the R console.
```{r,warning=FALSE}
