...
GWAStest<- function(phenotypes=NULL, genotypes=NULL, Cov=NULL, GM=NULL, PCA.M=3, QTN.position=NULL, cutoff=NULL){
  ...
  #Variance Explained
  var_exp_plot_data <- data.frame(t(PCA_results$importance)) # I transposed the data to be in long form and coerce to a data.frame for ggplot2::ggplot
  ...
  var_exp_plot <- ggplot2::ggplot(data = var_exp_plot_data, aes(x = pca_index, y = var_exp)) +
    geom_line() +
    geom_point() +
    labs(x = "PCA component index", y = "Variance explained", title = "Variance Explained")

  #Cumulative Variance Explained
  cum_var_exp_plot <- ggplot2::ggplot(data = var_exp_plot_data, aes(x = pca_index, y = cum_var_exp)) +
    geom_line() +
    geom_point() +
  ...
  pca_comp_plot_12 <-
    ggplot2::ggplot(data = PCA_plot_data, aes(x = PC1, y = PC2)) +
    geom_point()
  pca_comp_plot_13 <-
    ggplot2::ggplot(data = PCA_plot_data, aes(x = PC1, y = PC3)) +
    geom_point()
  pca_comp_plot_23 <-
  ...
  for (i in 1:m){
    ...
    # IF MAX NOT EQUAL TO MIN
      ...
      }else{fD <- mgcv::fixDependence(PCA$x[,1:PCA.M], as.matrix(CV))
...
<<<<<<< HEAD
  ...
  manhattan_plot <- function(marker_map, pvals,cutoff,QTN_index = c(), trait = "unknown")
  {
    ...
    manhattan_plot <- ggplot2::ggplot(marker_map, aes(x = 1:nrow(marker_map), y = pvals, color = factor(Chromosome))) +
      geom_point() +
